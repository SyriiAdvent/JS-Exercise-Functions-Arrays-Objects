...
function getCarInfoByIndex(inventory, index) {
  // console.log(`${inventory[index].car_make} ${inventory[index].car_model}`);
...
function getLastCarInfo(inventory) {
  let lastItem = inventory.length - 1;
  // console.log(lastItem);
  return `This is a ${inventory[lastItem].car_make} ${inventory[lastItem].car_model}`;
}
...
function getCarInfoById(inventory, id) {
  ...
  // console.log(id); // ID is 1

  let idLoc = id - 1;
  return `This is a ${inventory[idLoc].car_make} ${inventory[idLoc].car_model}`;
}
...
function sortCarInventory(inventory) {
  return inventory.sort((one, two) => {
    if (one.car_model > two.car_model) {
      return 1;
    } else {
      return -1;
    }
